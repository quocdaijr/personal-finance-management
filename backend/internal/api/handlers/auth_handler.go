...
func (h *AuthHandler) VerifyTwoFactor(c *gin.Context) {
	...
	token, err := h.authService.VerifyTwoFactor(&req)
	if err != nil {
		c.JSON(http.StatusUnauthorized, gin.H{"error": err.Error()})
		return
	}
...
// SetupTwoFactor handles 2FA setup
func (h *AuthHandler) SetupTwoFactor(c *gin.Context) {
	...
	// Setup 2FA
	response, err := h.authService.SetupTwoFactor(userID.(uint))
	if err != nil {
...
// DisableTwoFactor handles 2FA disabling
func (h *AuthHandler) DisableTwoFactor(c *gin.Context) {
	...
	// Disable 2FA
	err := h.authService.DisableTwoFactor(userID.(uint))
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// GetProfile handles getting the user profile
